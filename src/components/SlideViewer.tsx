...
class SlideViewer extends React.Component<SlideViewerProps, SlideViewerState> {
  ...
  constructor(props: SlideViewerProps) {
    ...
    this.state = {
      ...
      findingsOptions: [] as dcmjs.sr.coding.CodedConcept[],
  ...
  componentDidUpdate(
    ...
    if (
      ...
      // after re-constructing viewers:
      this.buildMagnificationMenu();
    }
  }
  ...
  getUniqueHoveredRois = (newRoi: dmv.roi.ROI | null): dmv.roi.ROI[] => {
    ...
    return uniqueIds
      .map((id) => allRois.find((roi) => roi.uid === id))
    ...
    if (
      ...
      if (
        ...
        this.setState((state) => {
          ...
          if (
            state.selectedPresentationStateUID === null ||
            state.selectedPresentationStateUID === undefined
          ) {
            const style = {
  ...
  // Cancel in-flight searches across keystrokes
  private findAbort?: AbortController;

  // 1) Add a small normalizer inside SlideViewer (near other handlers)
  private normalizeOntologyItem = (it: { code: string; name: string }) => {
    const raw = it.code ?? "";
    const name = it.name ?? raw;

    if (/^SNOMEDCT/i.test(raw)) {
      const m = raw.match(/(\d{6,})$/);
      const value = m ? m[1] : raw.replace(/^SNOMEDCT(_US)?_?/i, "");
      return new dcmjs.sr.coding.CodedConcept({
        value,
        meaning: name,
        schemeDesignator: "SCT",
      });
    } else if (/^MSH_/i.test(raw)) {
      const value = raw.replace(/^MSH_/i, "");
      return new dcmjs.sr.coding.CodedConcept({
        value,
        meaning: name,
        schemeDesignator: "MSH",
      });
    } else {
      return new dcmjs.sr.coding.CodedConcept({
        value: raw,
        meaning: name,
        schemeDesignator: "99MIDAS",
      });
    }
  };

  // Small fetcher
  private fetchFindingsPage = async (
    q: string,
    page: number,
    pageSize: number
  ) => {
    ...
    const items = (json?.data ?? []).map((it) =>
      this.normalizeOntologyItem(it)
    );
    const total = json?.count ?? items.length;
    return { items, total };
  };
  // Debounced server-side search against your ontology endpoint
  // Replace your searchFindings with this version
  private searchFindings = (value: string) => {
    this.setState({
      findingQuery: value,
      isFindingLoading: true,
      findingsOptions: [], // clear old list while loading
      findingPage: 1,
      hasMoreFindings: false,
    });
    if (this.findAbort) this.findAbort.abort();
    this.findAbort = new AbortController();
    clearTimeout((this as any)._findTimer);

    (this as any)._findTimer = setTimeout(async () => {
      try {
        ...
        // seed geometry defaults
        items.forEach((cc) => {
          const key = buildKey(cc);
          if (!this.geometryTypeOptions[key])
            this.geometryTypeOptions[key] = ["polygon", "freehandpolygon"];
        });
        this.setState({
          findingsOptions: items,
          isFindingLoading: false,
          hasMoreFindings: hasMore,
          findingPage: 1,
        });
      } catch {
        this.setState({
  ...
  private onFindingPopupScroll = async (e: React.UIEvent<HTMLDivElement>) => {
    const t = e.currentTarget;
    const nearBottom = t.scrollTop + t.clientHeight >= t.scrollHeight - 8;
    if (!nearBottom) return;
    this.loadMoreFindings();
  };

  private loadMoreFindings = async () => {
    const {
      isFindingLoading,
      isFindingLoadingMore,
      hasMoreFindings,
      findingPage,
      findingPageSize,
      findingQuery,
    } = this.state;
    if (isFindingLoading || isFindingLoadingMore || !hasMoreFindings) return;

    this.setState({ isFindingLoadingMore: true });
    try {
      ...
      // dedupe by scheme|value
      const seen = new Set(
        this.state.findingsOptions.map(
          (cc) => `${cc.CodingSchemeDesignator}|${cc.CodeValue}`
        )
      );
      const add = items.filter(
        (cc) => !seen.has(`${cc.CodingSchemeDesignator}|${cc.CodeValue}`)
      );
      add.forEach((cc) => {
        const key = buildKey(cc);
        if (!this.geometryTypeOptions[key])
      ...
      const loaded = nextPage * findingPageSize;
      const hasMore = loaded < total;

      this.setState({
        findingsOptions: [...this.state.findingsOptions, ...add],
        findingPage: nextPage,
        hasMoreFindings: hasMore,
        isFindingLoadingMore: false,
      });
    } catch {
      this.setState({ isFindingLoadingMore: false, hasMoreFindings: false });
    }
  };

  // Set Slim's selected finding and ensure geometry options exist
  private onFindingSelect = (codeValue: string) => {
    const cc = this.state.findingsOptions.find(
      (o) => o.CodeValue === codeValue
    );
    if (!cc) return;

    // Build a stable key exactly like other places in this file
    const key = buildKey(cc);
    if (!this.geometryTypeOptions[key])
      this.geometryTypeOptions[key] = ["polygon", "freehandpolygon"];
    if (!this.evaluationOptions[key]) this.evaluationOptions[key] = []; // ensure array for render
    this.setState({ selectedFinding: cc, selectedEvaluations: [] });
  };
  ...
  handleReportVerification = (): void => {
    ...
    if (this.state.generatedReport !== undefined) {
      ...
      // 1) Naturalized dataset produced by your SR builder
      const naturalized = this.state.generatedReport as any; // Comprehensive3DSR dataset [SR content]

      // 2) Denaturalize to DICOM element structure
      const denaturalized =
        dcmjs.data.DicomMetaDictionary.denaturalizeDataset(naturalized);

      // 3) Build File Meta and pass it to the DicomDict constructor
      const meta = {
      ...
      // 4) Create DicomDict with meta, assign dataset, and write Part 10 bytes
      const dict = new dcmjs.data.DicomDict(meta);
      dict.dict = denaturalized;
      const part10 = dict.write(); // ArrayBuffer with DICOM file bytes

      // 5) STOW-RS: send ArrayBuffer(s)
      client
        .storeInstances({ datasets: [part10] })
  ...
  private readonly getAnnotationConfigurations = (): React.ReactNode[] => {
    ...
    // REPLACE the hardcoded finding list with this async Select:
    annotationConfigurations.push(
      <>
        <div style={{ marginBottom: 8, fontWeight: 600 }}>Select finding</div>
        <Select
          showSearch
          allowClear
          ...
          style={{ width: 320 }}
          filterOption={false}
          onSearch={this.searchFindings}
          onChange={(v) => this.onFindingSelect(String(v))}
          // Spinner in the suffix icon area
          loading={this.state.isFindingLoading}
          // Replace list with loader during a new query
          notFoundContent={
            this.state.isFindingLoading ? <Spin size="small" /> : null
          }
          // Infinite scroll trigger
          onPopupScroll={this.onFindingPopupScroll}
          // Show a bottom loader while appending next page
          dropdownRender={(menu) => (
            <div>
              {menu}
              {this.state.isFindingLoadingMore ? (
          ...
          options={this.state.findingsOptions.map((cc) => ({
            value: cc.CodeValue,
            label: `${cc.CodeMeaning} (${cc.CodingSchemeDesignator}:${cc.CodeValue})`,
          }))}
    ...
    if (annotationGroups.length > 0) {
      ...
      const dropdownOptions = [
        ...
        ...Object.keys(annotationGroupsBySeries).map((seriesUID) => ({
          value: seriesUID,
          label: `${this.getSeriesDescription(seriesUID)} (${
            annotationGroupsBySeries[seriesUID]?.length ?? 0
          } groups)`,
        })),
      ];
  ...
  private readonly getToolbar = (): {
    ...
    const controlTools = [
      ...
      <Select
        size="middle"
        style={{ width: 80 }}
        value={this.state.selectedMagnification}
        onChange={(v) => this.setMagnification(Number(v))}
        options={this.state.availableMagnifications.map((m) => ({
          value: m.mag,
          label: m.label,
        }))}
      />,
  ...
  // Try to keep current view center in slide coords; otherwise fall back to slide center
  private getCurrentCenterXY = (): [number, number] => {
    // Prefer viewer’s center if available
    // @ts-expect-error optional in some builds
    const c = this.volumeViewer.getViewportCenter?.();
    if (Array.isArray(c) && c.length >= 2) return [Number(c), Number(c[1])];

    const [off, size] = this.volumeViewer.boundingBox; // [ [x0,y0], [w,h] ]
    return [off[0] + size[0] / 2, off[1] + size[1] / 2];
  };
